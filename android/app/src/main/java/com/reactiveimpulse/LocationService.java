...
public class LocationService extends ReactContextBaseJavaModule implements ActivityEventListener, OnCompleteListener<LocationSettingsResponse> {

    private static final String SELF_MODULE_NAME = "LocationService";
    ...
    public void onActivityResult(Activity activity, int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CHECK_SETTINGS && promise != null) {
            if (resultCode == RESULT_OK || isLocationProviderEnabled()) {
                ...
                promise.reject(ERR_USER_DENIED_CODE, new LocationServiceException("denied"));
            }
            this.promise = null;
        }
    }
            ...
            switch (exception.getStatusCode()) {
                case LocationSettingsStatusCodes.RESOLUTION_REQUIRED:
                    try {
                        ...
                        if (promise != null) promise.reject(ERR_FAILED_OPEN_DIALOG_CODE, new LocationServiceException("Failed to show dialog", sendEx));
                        this.promise = null;
                    } catch (ClassCastException classCast) {
                        if (promise != null) promise.reject(ERR_INTERNAL_ERROR, new LocationServiceException("Internal error", classCast));
                        this.promise = null;
                    }
                    break;
                case LocationSettingsStatusCodes.SETTINGS_CHANGE_UNAVAILABLE:
                    if (promise != null) promise.reject(ERR_SETTINGS_CHANGE_UNAVAILABLE_CODE, new LocationServiceException("Settings change unavailable"));
                    this.promise = null;
                    break;
            }
        }
